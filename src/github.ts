...
export async function getTeamCommits(
  token: string, org: string, authors: string[], sinceISO: string,
  onProgress?: (msg: string) => void,
): Promise<TeamResult> {
  log(`== GitHub metrics: org=${org} operadores=${authors.length} desde=${sinceISO} ==`)
  const repos = await listActiveRepos(token, org, sinceISO)
  onProgress?.(`${repos.length} repos activos`)
  ...
  for (const a of authors) {
    ...
    await pool(repos, CONCURRENCY, async (r) => {
      try { const cs = await repoCommits(token, r.owner, r.name, a, sinceISO); if (cs.length) { commits.push(...cs) } }
      catch (e: any) { err = e?.message || String(e); log(`autor ${a} repo ${r.name}: ${err}`) }
      done++
      if (done % 25 === 0) { onProgress?.(`${a}: ${done}/${repos.length} repos`) }
    })
